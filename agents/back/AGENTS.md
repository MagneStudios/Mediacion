...
- **`negociacion/`** — PR1 foundation, repositories only (no controller/service yet): `RondasRepository` (insert next ronda, read `casos.ronda_actual`), `PropuestasRepository`, `RespuestasRepository`, `propuesta-lock-query.ts`, and `ConfiguracionRepository.readIaConfig()` which reads `ia_modelo`/`ia_temperature`/`ia_max_tokens` from the `configuracion` table for the future AI engine. `propuestaViewColumns` defines the sanitized proposal read shape.
...
- **OpenRouter negotiation engine** — consumes `ConfiguracionRepository.readIaConfig()`; today only the repositories and config read exist.
