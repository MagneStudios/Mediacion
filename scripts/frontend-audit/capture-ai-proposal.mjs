...
const outDir = path.resolve('..', '..', 'docs', 'frontend-redesign', 'implementation-validation', 'ai-proposal');
const viewports = {
...
async function clickButtonByText(page, text) {
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    try {
      const loc = page.getByText(text, { exact: true }).locator('visible=true').first();
...
try {
  ...
  await m.page.goto(`${BASE_URL}/case/case-2/negotiation`, { waitUntil: 'networkidle' });
...
try {
  ...
  await page.getByPlaceholder('Por ejemplo, organización familiar').waitFor({ state: 'visible', timeout: 15000 });
  await page.getByPlaceholder('Por ejemplo, organización familiar').fill(caseName);
  await page.waitForTimeout(500);
  await clickButtonByText(page, 'Continuar');

  await page.waitForTimeout(800);
  await page.getByRole('radio', { name: 'Mediación' }).waitFor({ state: 'visible', timeout: 15000 });
  await page.getByRole('radio', { name: 'Mediación' }).click({ timeout: 15000 });
  await page.waitForTimeout(500);
  await clickButtonByText(page, 'Continuar');

  await page.waitForTimeout(800);
  await page.getByText('Revisá la información').waitFor({ state: 'visible', timeout: 15000 });
...
try {
  ...
  await page.goto(`${BASE_URL}/case/case-1/positions/create`, { waitUntil: 'networkidle' });
  await page.waitForTimeout(1000);
  await page.getByRole('radio', { name: 'Otro tema', exact: true }).waitFor({ state: 'visible', timeout: 15000 });
  await page.getByRole('radio', { name: 'Otro tema', exact: true }).click();
  await page.waitForTimeout(300);
  await page.getByPlaceholder('Por ejemplo, días de cuidado durante la semana').fill('Calendario compartido (QA)');
  await page.getByLabel('Límite mínimo que considerarías').fill('Alternancia semanal');
  await page.getByLabel('Límite máximo que considerarías').fill('Alternancia quincenal');
  await page.getByRole('radio', { name: 'Podría ceder en este tema', exact: true }).click();
  await page.waitForTimeout(300);
  await clickButtonByText(page, 'Continuar');
  await page.waitForTimeout(800);
