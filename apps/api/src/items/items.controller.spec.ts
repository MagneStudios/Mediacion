...
describe("ItemsController unit", () => {
  it("creates an item using the caso id, caller id and body", async () => {
    const createOwnItem = jest.fn().mockResolvedValue({
      id: "item-1",
      caso_id: "caso-1",
      parte_id: "user-a",
    });
    ...
    const result = await controller.createItem("caso-1", parteA, dto);

    expect(createOwnItem).toHaveBeenCalledWith("caso-1", "user-a", dto);
  ...
  it("lists items using the caso id and caller id", async () => {
    const ownItems = [{ id: "item-1", caso_id: "caso-1", parte_id: "user-a" }];
    const listOwnItems = jest.fn().mockResolvedValue(ownItems);
    const controller = new ItemsController({
      createOwnItem: jest.fn(),
      listOwnItems,
      getOwnItem: jest.fn(),
      updateOwnItem: jest.fn(),
    } as unknown as ItemsService);
    ...
    expect(listOwnItems).toHaveBeenCalledWith("caso-1", "user-a");
    expect(result).toBe(ownItems);
  });

  it("gets a single item using the item id and caller id", async () => {
    const item = { id: "item-1", parte_id: "user-a" };
    const getOwnItem = jest.fn().mockResolvedValue(item);
    const controller = new ItemsController({
      createOwnItem: jest.fn(),
      listOwnItems: jest.fn(),
      getOwnItem,
    ...
    expect(getOwnItem).toHaveBeenCalledWith("item-1", "user-a");
    expect(result).toBe(item);
  });

  it("updates a single item using the item id, caller id and body", async () => {
    const updated = { id: "item-1", parte_id: "user-a", nombre: "Bici" };
    const updateOwnItem = jest.fn().mockResolvedValue(updated);
    const controller = new ItemsController({
      createOwnItem: jest.fn(),
      listOwnItems: jest.fn(),
      getOwnItem: jest.fn(),
    ...
    const patch: UpdateItemDto = { nombre: "Bici" };

    const result = await controller.updateItem("item-1", parteA, patch);

    expect(updateOwnItem).toHaveBeenCalledWith("item-1", "user-a", patch);
