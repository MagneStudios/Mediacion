...
describe('buildAgreementState — authoritative completed estado', () => {
  it('con_aviso with all signers pendiente → allSignaturesComplete: true, not waiting', () => {
    const agreement = makeAgreement({ estado: 'con_aviso' });
    const signers = makeSigners([
      { role: 'authenticated_party', status: 'pendiente' },
      { role: 'other_party', status: 'pendiente' },
  ...
  it('con_aviso with own firmado but other pendiente → allSignaturesComplete: true, not waiting', () => {
    const agreement = makeAgreement({ estado: 'con_aviso' });
    const signers = makeSigners([
      { role: 'authenticated_party', status: 'firmado' },
      { role: 'other_party', status: 'pendiente' },
  ...
  it('firmado with all signers pendiente → allSignaturesComplete: true, not waiting', () => {
    const agreement = makeAgreement({ estado: 'firmado' });
    const signers = makeSigners([
      { role: 'authenticated_party', status: 'pendiente' },
      { role: 'other_party', status: 'pendiente' },
  ...
  it('firmado with own pendiente but other firmado → allSignaturesComplete: true, not waiting', () => {
    const agreement = makeAgreement({ estado: 'firmado' });
    const signers = makeSigners([
      { role: 'authenticated_party', status: 'pendiente' },
      { role: 'other_party', status: 'firmado' },
  ...
  it('enviado_a_firma with own signed and other pending → still waiting (not a completed estado)', () => {
    const agreement = makeAgreement({ estado: 'enviado_a_firma' });
    const signers = makeSigners([
      { role: 'authenticated_party', status: 'firmado' },
      { role: 'other_party', status: 'pendiente' },
