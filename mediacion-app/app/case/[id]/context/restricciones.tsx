...
import { RestrictionsSectionFields } from '@/features/case-context/components/RestrictionsSectionFields';
...
export default function RestriccionesScreen() {
  ...
  const items = draft?.restricciones ?? [];

  const handleChange = (newItems: CaseContextEntry<Restriction>[]) => {
    void queueSave(caseId, 'restricciones', newItems);
  };
    ...
    >
      ...
      <View style={styles.header}>
        <Text style={styles.title} accessibilityRole="header">
          {t('caseContext.sections.restricciones.title')}
        </Text>
        <Text style={styles.subtitle}>{t('caseContext.sections.restricciones.description')}</Text>
      </View>

      <PrivacyNotice />

      <RestrictionsSectionFields items={items} onChange={handleChange} />
