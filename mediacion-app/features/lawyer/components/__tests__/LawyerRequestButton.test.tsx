...
describe('LawyerRequestButton', () => {
  ...
  describe('once the scope is published', () => {
    ...
    it('abre el checkout y publica la solicitud creada cuando el backend devuelve una URL', async () => {
      ...
      mockRequestLawyer.mockResolvedValue({
        ...
        checkoutUrl: 'https://mp/checkout/1',
      });

      await render(
        <I18nextProvider i18n={i18n}>
          <LawyerRequestButton casoId="case-1" onRequested={onRequested} />
        </I18nextProvider>,
      );
      fireEvent.press(screen.getByText(i18n.t('lawyer.action')));
      await screen.findByText(i18n.t('lawyer.dialog.title'));
      ...
      await waitFor(() => expect(openURL).toHaveBeenCalledWith('https://mp/checkout/1'));
      expect(onRequested).toHaveBeenCalledWith(expect.objectContaining({ id: 'lawreq-0001' }));
      openURL.mockRestore();
    });
