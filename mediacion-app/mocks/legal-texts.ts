/**
 * Verbatim legal texts for the mock `legal_documents` rows — generated from
 * `docs/TYC_Metodos_autocompositivos.md` and `docs/metodos_privacidad_raw.md`
 * (Golosetti Abogados deliverables, merged in PR #100). Do not edit by hand:
 * the source of truth for the wording is the docs/ pair, and the real one will
 * be the `legal_documents` table once DB seeds it.
 *
 * The [COMPLETAR: …] placeholders are intentionally kept visible: they are
 * Administración's pending inputs (razón social, CUIT, domicilio, casillas…)
 * and hiding them behind pretty placeholder copy would hide the blocker.
 *
 * Format: "## X. TITLE" lines are section headings; every other non-blank
 * line is a paragraph. That is the entire grammar LegalDocumentBody parses.
 */

export const termsV1Text = `Los presentes Términos y Condiciones Generales de Uso y Contratación (en adelante, los “Términos y Condiciones”) regulan el acceso y la utilización de la Plataforma por parte del Usuario, así como la contratación de los Servicios a través de ella. La Plataforma es de propiedad de [COMPLETAR: razón social y tipo societario], CUIT [COMPLETAR: CUIT], con domicilio en [COMPLETAR: calle, número, localidad y provincia] (en adelante, la “Empresa”), y se encuentra disponible en [COMPLETAR: URL del sitio web] y en la aplicación móvil [COMPLETAR: nombre de la aplicación móvil].
...
U.3. La presente Política se encuentra vigente a partir del [COMPLETAR: fecha]. Versión 1.0.`;
