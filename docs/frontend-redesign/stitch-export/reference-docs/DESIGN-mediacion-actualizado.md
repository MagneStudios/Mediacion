...
# Amendments (post token-implementation, phase 1 foundations)

- **`status-success` / `semantic-success` corrected from `#2F858B` to `#2F8F6B`.** The original value was identical to `mediation-aqua`, which would make a successful/agreed outcome visually indistinguishable from "this is AI-assisted" — this codebase has a standing, tested rule that the AI/mediation accent is reserved exclusively for AI-assisted actions and must never be reused for any other semantic meaning. Confirmed with the product owner; success now gets its own distinct green, kept in the same muted/cool family as the rest of the palette. `mediation-aqua`/Turquesa remains exclusively the AI/mediation accent; `accent`/Cerúleo remains exclusively focus/info — the three never overlap.
- The implementation additionally derives darker, contrast-verified text shades for `successFg`/`warningFg`/`errorFg` when paired with their light `*Bg` tints (`#1F6E4F`/`#7A4B0B`/`#8E2632` respectively) — the base tokens above land at 3.3–4.0:1 against those backgrounds, which clears only the 3:1 non-text bar, not the 4.5:1 WCAG AA bar for normal-size text. `warningFg`/`errorFg` reuse this doc's own `status-chip-warning`/`status-chip-danger` component text values for exactly this reason; `success` had no such chip defined here, so its darker shade was derived the same way and should be added to this doc's `components:` block as `status-chip-success` text if that chip is ever built.
