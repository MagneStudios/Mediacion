...
> ~~**Y lo que pasó a ser lo primero que necesitamos, al 09/09:** los tres endpoints de `docs/pedidos-frontend-acuerdos-modulares.md` (`GET /acuerdos/:id`, `subject_type`+`version` en `GET /firmas`, `GET /casos/:id/negociaciones`) más los §7.1 y §7.2 de `docs/pedidos-frontend-a-backend-recolocar-negociaciones.md`. **Son lo único que frena el refactor de acuerdos modulares**: DB entregó el modelo el 06/09.~~ **Al 10/09: Backend entregó los cuatro (más `renegociar`) el 09/09 a las 16:53**, y este párrafo se escribió sin ese merge. `GET /acuerdos/:id` y los campos de `/firmas` ya se consumen; la lista de negociaciones y `renegociar` van en el PR siguiente.
