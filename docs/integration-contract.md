...
>
> ~~Nota aparte: Backend mencionó un `docs/pedidos-backend-a-frontend-acuerdos-modulares.md`. **Ese archivo no existe** — ni en `dev`, ni en ninguna rama remota, ni en la historia del repo. Si lo escribió, falta pushearlo.~~ **Retractado el 10/09: el archivo existe desde el 09/09 16:46.** Esta nota se escribió contra un árbol de trabajo anterior al merge de Backend de las 16:53 — el mismo error que corrige el banner de `auditoria-desbloqueos-09-09-2026.md`. Los cuatro endpoints que ese doc lista están en `dev`; `GET /acuerdos/:id` y los dos campos de `GET /firmas` se consumen desde el 10/09 (`docs/changelogs/2026-09-10.md`).

### Consumido por `mediacion-app`

`GET/POST /casos` · `GET /casos/:id` · `POST /casos/:id/invitaciones` · **`GET /casos/:id/invitaciones`** · `POST /casos/unirse` · `GET /casos/:casoId/actividad` · los cinco de `items` · los tres de `negociacion` · **`GET /casos/:casoId/negociaciones`** · **`POST /negociaciones/:id/renegociar`** · `GET/POST /casos/:casoId/acuerdo` · **`GET /acuerdos/:id`** · `POST /acuerdos/:id/firmar` · `GET /firmas` (**con `subject_type` y `version` desde el 10/09**) · **`POST/GET /acuerdos/:id/incumplimiento(s)`** · **`GET /acuerdos/:id/exportar`** · `GET /casos/:casoId/mediacion` · `GET /casos/:casoId/mediadores` · `POST /casos/:casoId/mediacion` · los cuatro de `notificaciones` · los cinco de `/me` · los seis públicos/bearer de `legal` · **`GET /planes`** · `GET /suscripciones/vigente` · **`GET /suscripciones/uso`** · `POST /suscripciones/:id/baja` · **`GET /casos/:casoId/tareas`** · **`PATCH /tareas/:id`**

En **negrita**, lo integrado el 25/08 — salvo `GET /suscripciones/uso`, que se integró el 08/09 (`docs/changelogs/2026-09-08.md`), y `GET /acuerdos/:id` más los dos campos de `/firmas`, el 10/09 (`docs/changelogs/2026-09-10.md`), y la lista de negociaciones más `renegociar`, el mismo día (`docs/changelogs/2026-09-10-negociaciones-por-materia.md`). Detalle del resto en `docs/changelogs/2026-08-25.md`.
...
| `PATCH /casos/:id/plazo` · `PATCH /casos/:id/estado` | ✅ **Consumidos desde el 09/09** (`docs/changelogs/2026-09-09-plazo-y-terminacion.md`). ~~No hay pedido de Producto~~ **Sí lo había, y era fundacional.** `Mediacion_Documentacion_Tecnica_v1_0.md:319-320` los lista con rol **Parte** — no panel — como **RN-10** (*"una parte puede fijar un plazo puntual"*) y **RN-08** (*"cualquiera de las partes puede declarar expresamente el fin de una negociación"*). Nuestro propio `frontend-redesign/state-machines.md:14` ya modela la transición. Fijar un plazo es además lo que le da sentido al semáforo, que hoy dibujamos como decoración de sólo lectura | No bloquea a BE — **es trabajo de FE que nadie asignó**, y necesita un date picker que el design system no tiene |
...
> ~~**Y lo que pasó a ser lo primero que necesitamos, al 09/09:** los tres endpoints de `docs/pedidos-frontend-acuerdos-modulares.md` (`GET /acuerdos/:id`, `subject_type`+`version` en `GET /firmas`, `GET /casos/:id/negociaciones`) más los §7.1 y §7.2 de `docs/pedidos-frontend-a-backend-recolocar-negociaciones.md`. **Son lo único que frena el refactor de acuerdos modulares**: DB entregó el modelo el 06/09.~~ **Al 10/09: Backend entregó los cuatro (más `renegociar`) el 09/09 a las 16:53**, y este párrafo se escribió sin ese merge. Los cuatro se consumen desde el 10/09.
...
| `POST /casos/unirse` | Bearer | `{token}` (`invitaciones.types.ts:18-20`) | `{id, estado}` of the joined caso (`invitaciones.types.ts:22`). **Token TTL: configurado** en `configuracion.invitacion_ttl_horas` (seed 72 h / R-04), leído en `joinCase` (`invitaciones.repository.ts`) con fallback `DEFAULT_INVITATION_TTL_HOURS = 72` (`invitation-ttl.ts`). Implementado 09-09 (cierra `auditoria-desbloqueos-09-09-2026.md` §1); email invitations enforce caller-email match |
...
| Counterparty join screen (enter code / open link) | FRONTEND-NOT-WIRED | Backend `POST /casos/unirse {token}` is fully built (transactional join, RN-07 activation `nuevo→activo`, email match, config TTL -- 72 h via `configuracion.invitacion_ttl_horas`, anti-race lock — `invitaciones.controller.ts:28-38`). **No app screen or deep-link route exists to consume it.** This is the biggest missing frontend flow. |
