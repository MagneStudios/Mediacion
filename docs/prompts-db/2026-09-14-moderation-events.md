# Prompt DB — Trazabilidad de moderación de lenguaje
...
- Tabla **server-only**: `service_role` escribe; `public.is_admin()` lee; `anon`/`authenticated`
  no tienen SELECT (RLS deniega por defecto y no se otorga GRANT de SELECT salvo para admin vía RLS).
- Grants explícitos por objeto (ver `20260817130000_rate_limit_counters.sql` y
  `20260821120000_monetizacion_fase1.sql:265` para el patrón server-only).
- Admin = `public.is_admin()`.

## 1) Crear `supabase/migrations/20260914130000_moderation_events.sql`
...
Agregá `moderation_events` en `public` (Row/Insert/Update/Relationships). Notá que NO tiene
`updated_at`. UUID → `string`; `timestamptz` → `string`; `jsonb` → `unknown`.

## 3) `docs/database.md`
Agregá subsección "Moderación de lenguaje (trazabilidad)" indicando que es server-only
(solo `service_role` escribe, solo `is_admin()` lee).
